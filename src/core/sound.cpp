...
HEFFECT HGE_Impl::Effect_Load(const char *filename, DWORD size)
{
...
HCHANNEL HGE_Impl::Effect_Play(HEFFECT eff)
{
	if(hBass)
	{
		HCHANNEL chn;
...
HCHANNEL HGE_Impl::Effect_PlayEx(HEFFECT eff, int volume, int pan, float pitch, bool loop)
{
	if(hBass)
	{
		BASS_SAMPLE info;
...
void HGE_Impl::Effect_Free(HEFFECT eff)
{
	if(hBass) BASS_SampleFree(eff);
}


HMUSIC HGE_Impl::Music_Load(const char *filename, DWORD size)
{
	void *data;
	DWORD _size;
	HMUSIC hm;
...
HCHANNEL HGE_Impl::Music_Play(HMUSIC mus, bool loop, int volume, int order, int row)
{
	if(hBass)
	{
...
void HGE_Impl::Music_Free(HMUSIC mus)
{
	if(hBass) BASS_MusicFree(mus);
}

void HGE_Impl::Music_SetAmplification(HMUSIC music, int ampl)
{
	if(hBass) BASS_MusicSetAttribute(music, BASS_MUSIC_ATTRIB_AMPLIFY, ampl);
}

int HGE_Impl::Music_GetAmplification(HMUSIC music)
{
	if(hBass) return BASS_MusicGetAttribute(music, BASS_MUSIC_ATTRIB_AMPLIFY);
	else return -1;
}

int HGE_Impl::Music_GetLength(HMUSIC music)
{
	if(hBass)
	{
...
void HGE_Impl::Music_SetPos(HMUSIC music, int order, int row)
{
	if(hBass)
	{
...
bool HGE_Impl::Music_GetPos(HMUSIC music, int *order, int *row)
{
	if(hBass)
	{
		DWORD pos;
...
void HGE_Impl::Music_SetInstrVolume(HMUSIC music, int instr, int volume)
{
	if(hBass)
	{
...
int HGE_Impl::Music_GetInstrVolume(HMUSIC music, int instr)
{
	if(hBass)
	{
...
void HGE_Impl::Music_SetChannelVolume(HMUSIC music, int channel, int volume)
{
	if(hBass)
	{
...
int HGE_Impl::Music_GetChannelVolume(HMUSIC music, int channel)
{
	if(hBass)
	{
...
HSTREAM HGE_Impl::Stream_Load(const char *filename, DWORD size)
{
	void *data;
	DWORD _size;
	HSTREAM hs;
...
void HGE_Impl::Stream_Free(HSTREAM stream)
{
...
HCHANNEL HGE_Impl::Stream_Play(HSTREAM stream, bool loop, int volume)
{
	if(hBass)
	{
		BASS_CHANNELINFO info;
...
void HGE_Impl::Channel_SetPanning(HCHANNEL chn, int pan)
{
	if(hBass) BASS_ChannelSetAttributes(chn, -1, -1, pan);
}

void HGE_Impl::Channel_SetVolume(HCHANNEL chn, int volume)
{
	if(hBass) BASS_ChannelSetAttributes(chn, -1, volume, -101);
}

void HGE_Impl::Channel_SetPitch(HCHANNEL chn, float pitch)
{
	if(hBass)
	{
		BASS_CHANNELINFO info;
...
void HGE_Impl::Channel_Pause(HCHANNEL chn)
{
	if(hBass) BASS_ChannelPause(chn);
}

void HGE_Impl::Channel_Resume(HCHANNEL chn)
{
	if(hBass) BASS_ChannelPlay(chn, FALSE);
}

void HGE_Impl::Channel_Stop(HCHANNEL chn)
{
	if(hBass) BASS_ChannelStop(chn);
}

void HGE_Impl::Channel_PauseAll()
{
	if(hBass) BASS_Pause();
}

void HGE_Impl::Channel_ResumeAll()
{
	if(hBass) BASS_Start();
}

void HGE_Impl::Channel_StopAll()
{
	if(hBass)
	{
		BASS_Stop();
...
bool HGE_Impl::Channel_IsPlaying(HCHANNEL chn)
{
	if(hBass)
	{
...
float HGE_Impl::Channel_GetLength(HCHANNEL chn) {
	if(hBass)
	{
...
float HGE_Impl::Channel_GetPos(HCHANNEL chn) {
	if(hBass)
	{
...
void HGE_Impl::Channel_SetPos(HCHANNEL chn, float fSeconds) {
	if(hBass)
	{
...
void HGE_Impl::Channel_SlideTo(HCHANNEL channel, float time, int volume, int pan, float pitch)
{
	if(hBass)
	{
		BASS_CHANNELINFO info;
...
bool HGE_Impl::Channel_IsSliding(HCHANNEL channel)
{
	if(hBass)
	{
