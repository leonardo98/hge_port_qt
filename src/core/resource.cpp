...
bool HGE_Impl::Resource_AttachPack(const char *filename, const char *password)
{
	char *szName;
...
void HGE_Impl::Resource_RemovePack(const char *filename)
{
	char *szName;
...
void HGE_Impl::Resource_RemoveAllPacks()
{
...
void* HGE_Impl::Resource_Load(const char *filename, DWORD *size)
{
...
void HGE_Impl::Resource_Free(void *res)
{
	if(res) free(res);
}


char* HGE_Impl::Resource_MakePath(const char *filename)
{
	int i;
...
char* HGE_Impl::Resource_EnumFiles(const char *wildcard)
{
	if(wildcard)
	{
...
char* HGE_Impl::Resource_EnumFolders(const char *wildcard)
{
	if(wildcard)
	{
		if(hSearch) { FindClose(hSearch); hSearch=0; }
